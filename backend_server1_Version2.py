# backend_server1.py — 後端：Socket 接收 + 偵測 + 雙路 MJPEG + LINE自動推播
from flask import Flask, Response, jsonify
import socket, struct, threading, time, traceback
import cv2, numpy as np
from collections import deque
from fall_detection1 import process_frame  # 回傳 (fall_detected, annotated_frame)
import requests  # <--- 加入 requests 用於 LINE 通知
...
# ======== LINE Webhook 設定 ========
LINE_WEBHOOK_URL = "http://localhost:4000"

def get_all_user_ids():
    try:
        r = requests.get(f"{LINE_WEBHOOK_URL}/userIds")
        data = r.json()
        return data.get("userIds", [])
    except Exception as e:
        print(f"無法取得 userIds: {e}")
        return []

def notify_users(user_ids, msg):
    for uid in user_ids:
        try:
            res = requests.post(
                f"{LINE_WEBHOOK_URL}/notify",
                json={"userId": uid, "message": msg}
            )
            print(f"[LINE] 已通知 {uid}，狀態碼 {res.status_code}")
        except Exception as ex:
            print(f"[WARN] LINE 通知失敗: {ex}")
...
def fall_detection_thread():
    ...
    while True:
        ...
        try:
            ...
            # === 跌倒推播（只推播第一次，跌倒解除才會重設）===
            if fall_detected and not notified:
                print("[INFO] 檢測到跌倒！通知 LINE 用戶...")
                user_ids = get_all_user_ids()
                notify_users(user_ids, "⚠️ 偵測到跌倒！請立即處理！")
                notified = True
            elif not fall_detected:
                notified = False  # 跌倒解除才可再次通知
